    ...
    <div class="card-body">
        ...
        <div class="mb-2">
            <form method="GET" class="form-inline">
                <input type="text" name="lesson_search" value="{{ e($lessonSearch) }}"
                    class="p-2 form-control form-control-sm mb-2" placeholder="Search Lessons">
                ...
                @if ($lessonSearch || $lessonPage)
                    <a href="{{ url()->current() }}{{ $lessonResetQuery }}" class="btn btn-sm btn-secondary">Reset
                        Lessons</a>
                @endif
            </form>
        </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($lessons as $lesson)
                            <tr>
                                <td>{{ $serial++ }}</td>
                                <td><a
                                        href="{{ backpack_url('lesson/' . $lesson->id . '/show') }}">{{ e($lesson->id) }}</a>
                                </td>
